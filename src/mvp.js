(() => {
  ...
  function layout() {
    ...
    for (const ch of Array.from(text)) {
      ...
      result.push({
        id: 'g_' + Date.now().toString(36) + '_' + idx++,
        char: ch,
        glyphIndex: glyph.index,
        x: x + geo.width / 2 + xJitter,
        y,
        rotation,
        scaleX,
        scaleY,
        ...
        instanceBias,
        fill: PRESET_FALLBACK_COLOR,
        strokeWidth,
        geo,
        geoSeed: seed,
        geoStrength: strength,
        geoBias: instanceBias
      });
  ...
  function invPoint(g, px, py) {
    ...
    const dy = py - g.y;
    return {
      x: (dx * c - dy * s) / g.scaleX + g.geo.width / 2,
      y: (dx * s + dy * c) / g.scaleY + g.geo.height / 2
    };
  }
      ...
      btn.onclick = () => {
        const next = glyphs.map(x => {
          ...
          return {
            ...
            geo: newGeo,
            geoSeed: c.seed,
            geoStrength: x.warpStrength,
            geoBias: x.instanceBias,
            geoSize: x.fontSize,
            y: preserveBaselineY(x, newGeo)
          };
        });
  ...
  canvas.addEventListener('pointerdown', e => {
    ...
    if (hitRotate(p.x, p.y) && selected.size) {
      ...
      rotateState = {
        cx: sb.cx,
        cy: sb.cy,
        start: Math.atan2(p.y - sb.cy, p.x - sb.cx),
        snapshot: glyphs.filter(g => selected.has(g.id)).map(g => ({ ...g }))
      };
      pointerMode = 'rotate';
      return;
    }
  ...
  canvas.addEventListener('pointermove', e => {
    ...
    if (pointerMode === 'select') {
      selectRect = rectNorm(pointerStart, p);
      draw();
      return;
    }
  ...
  canvas.addEventListener('pointerup', e => {
    ...
    if (pointerMode === 'drag' && dragSnapshot) {
      ...
      if (moved) {
        ...
        for (const s of dragSnapshot) {
          const item = before.find(x => x.id === s.id);
          if (item) {
            item.x = s.x;
            item.y = s.y;
          }
    ...
    if (pointerMode === 'select' && selectRect) {
      ...
      if (!add) selected.clear();
      for (const g of glyphs) {
        if (intersects(selectRect, glyphBox(g))) selected.add(g.id);
      }
    }
  ...
  function applyStyleChanges() {
    ...
    const next = glyphs.map(g => {
      ...
      return {
        ...
        fontSize: targetSize,
        geo: newGeo,
        geoSeed: g.variantSeed,
        geoStrength: g.warpStrength,
        geoBias: g.instanceBias,
        geoSize: targetSize,
        y: preserveBaselineY(g, newGeo)
      };
    });
  ...
  els.thicknessInput.oninput = () => {
    els.thicknessText.textContent = Number(els.thicknessInput.value).toFixed(2) + 'px';
  };
  els.sizeInput.oninput = () => {
    els.sizeText.textContent = Math.round(+els.sizeInput.value) + 'px';
  };
